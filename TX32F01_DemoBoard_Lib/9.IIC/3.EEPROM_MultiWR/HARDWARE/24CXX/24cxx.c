...
//ReadAddr:开始读数的地址
//返回值  :读到的数据
u8 AT24CXX_ReadMulti(u16 ReadAddr,u8* Datatmp,u8 Len)
{
    u8 temp=0,i;
    ...
    {
        I2C_SEND_ACK();//发送ACK
        if(truefp != I2C_Sta_Check(I2C_STA_DATARECVED_ACK)) 
    {
        //等待SI跳转
            I2C_GenerateSTOP();//检查状态失败，发送STOP后返回
            return falsefp;
        }
        Datatmp[i] = I2C_ReceiveData();//接收数据
    }
...
//在AT24CXX指定地址写入一串数据
//WriteAddr  :写入数据的目的首地址
//Datatmp:要写入的数据
//Len：数据长度,注意AT24C02一次性最多写入8字节数据
booltyped AT24CXX_WriteMulti(u16 WriteAddr,u8* Datatmp,u8 Len)
{
    u8 i;
    I2C_GenerateSTART();//产生START
    if(truefp != I2C_Sta_Check(I2C_STA_STARTBIT_SENDED)) 
    ...
    for(i=0; i<Len; i++)
    {
        I2C_SendData(Datatmp[i]);//发送要写入数据
        if(truefp != I2C_Sta_Check(I2C_STA_DATW_ACKED)) 
        {
            //等待SI跳转
            I2C_GenerateSTOP();//检查状态失败，发送STOP后返回
...
#elif defined Multi_WR
    //8个字节为1页
    u16 i,Page=NumToRead/8+1;

    if(NumToRead%8) Page-=1;

    for(i=0; i<Page; i++)
    {
        AT24CXX_ReadMulti(ReadAddr+i*8,pBuffer,8);
        pBuffer+=8;
    }

    if(NumToRead%8)
    {
        AT24CXX_ReadMulti(ReadAddr+i*8,pBuffer,NumToRead%8);
    }
#endif
}
...
#elif defined Multi_WR
    //8个字节为1页
    u16 i,Page=NumToWrite/8+1;

    if(NumToWrite%8) Page-=1;

    for(i=0; i<Page; i++)
    {
        AT24CXX_WriteMulti(WriteAddr+i*8,pBuffer,8);
        pBuffer+=8;
    }

    if(NumToWrite%8)
    {
        AT24CXX_WriteMulti(WriteAddr+i*8,pBuffer,NumToWrite%8);
    }
#endif
}
