...
void Timer0_DelayMs(u32 ms)
{
    TIM_SetAutoreload(TIM0,ms-1);//定义延时时间
	TIM_ClearFlag(TIM0,TIM_IFR_CNTIF);//清除标志位
    TIM_Cmd(TIM0,ENABLE);//计数器使能
	
    while(!TIM_GetFlagStatus(TIM0,TIM_IFR_CNTIF));//等待标志位置位
    TIM_ClearFlag(TIM0,TIM_IFR_CNTIF);//清除标志位

    TIM_Cmd(TIM0,DISABLE);//计数器关闭
}
