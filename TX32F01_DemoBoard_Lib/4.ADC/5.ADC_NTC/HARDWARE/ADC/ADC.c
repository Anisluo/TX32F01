...
void ADC_SingleChannel_Init(u16 ADCVref,u8 Channel)
{
    ...
    SCU_PeriphClockCmd(Periph_GPIO0,ENABLE);//打开GPIO0外设时钟
    SCU_Lock();// 上锁SCU，不可对其余SCU寄存器操作。

    GPIO_Init(GPIO0,PIN02,GPIO_MODE_ANALOG);//选择P02为采样通道
    ...
    ADC_InitStructure.ADC_CLK_DIV=ADC_CLK_DIV_64;//ADC时钟选择系统时钟8分频
