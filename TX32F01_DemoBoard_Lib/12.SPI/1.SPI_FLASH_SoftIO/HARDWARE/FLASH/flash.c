...
void SPI_Flash_Init(void)
{
    ...
    GPIO_Init(GPIO2,PIN04,GPIO_MODE_OUTPUT_PP);//片选信号
    SPI_FLASH_CS(1);//取消片选

    GPIO_Init(GPIO2,PIN05,GPIO_MODE_AF);//配置为复用模式
    GPIO_Init(GPIO3,PIN00,GPIO_MODE_AF);//配置为复用模式
    GPIO_Init(GPIO3,PIN01,GPIO_MODE_AF);//配置为复用模式
